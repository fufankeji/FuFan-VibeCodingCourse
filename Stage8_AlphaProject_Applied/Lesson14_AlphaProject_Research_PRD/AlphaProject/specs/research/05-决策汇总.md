...
**fork 三件套** = `TauricResearch/TradingAgents`（上游内核同步）+ `hsliuping/TradingAgents-CN`（A 股中文化 + 后端 app/ 需商授）+ `chengzuopeng/stock-dashboard`（开源前端运行时，ISC stock-sdk 已实测核实）→ **Tushare Pro 500 元/年 + AkShare 免费 + MiniQMT** 数据 → **ClickHouse + Redis + PostgreSQL** 存储 → **TradingView Lightweight Charts**（替换 SD 的 ECharts）→ **飞书机器人**（重写借鉴 go-stock 告警思路，避 GPL）→ **腾讯云轻量 4C8G**。**A 轨（商授通过）MVP ~30 人日 / B 轨（商授失败）~40 人日，月成本 ~355 元**。
...
- **fork 3**：`chengzuopeng/stock-dashboard`（开源前端运行时，ISC stock-sdk）
...
| **前端来源** | SD（替换 CN 闭源 `frontend/`） | SD（同 A 轨） |
| **改造工作量** | **~30 人日** | **~40 人日** |
...
| **前端 Dashboard 运行时** | `chengzuopeng/stock-dashboard` | fork + `services/sdk.ts` 单点适配后端 endpoint | ISC（stock-sdk）+ 仓库 LICENSE 待补 |
...
- ⚠️ **license 待补**：stock-dashboard 仓库无 LICENSE 文件（问作者补，fallback 重写设计语言）
...
| **前端 UI** | **fork `chengzuopeng/stock-dashboard`**（React 19 + TS + Vite 7 基线）+ **TradingView Lightweight Charts**（替换其 ECharts）+ **SSE 主/WS 备** | v2 调整：放弃从零搭 Next.js，直接 fork SD 9 页面骨架（路由+组件+sdk.ts 适配层 1-2 人天对接后端）；K 线用 TV charts 性能碾压（35KB vs ECharts 200KB+） | 从零搭 Next.js + shadcn 浪费 SD 已就位的 9 页面骨架（自选股分组、个股详情、热力图、涨跌榜、EOD picker）；ECharts 高频掉帧 |
...
- ⚠️ **SD 仓库无 LICENSE 文件**：Day 1 邮件 chengzuopeng 询补全意愿；fallback 基于其设计语言重写（5-7 人日）
...
| 2 | 邮件 chengzuopeng 询 stock-dashboard 仓库 **LICENSE 补全意愿** | **Day 1** |
...
> 文档版本：**v2.0 · 2026-05-20**（v1 同日，对抗辩论后修订）
