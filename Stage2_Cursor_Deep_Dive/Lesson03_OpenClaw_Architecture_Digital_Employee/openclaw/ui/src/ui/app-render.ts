        ...
        ${
          state.tab === "models"
            ? renderModels({
                ...
                configForm: state.configForm,
                lastError: state.lastError,
                onLoad: () => loadConfig(state),
                onAdd: (modelName, baseUrl, apiKey) => {
                  const providerId = customProviderId(modelName);
                  const config = buildCustomProviderConfig(modelName, baseUrl, apiKey);
                  updateConfigFormValue(state, ["models", "providers", providerId], config);
                },
                onRemove: (id) => removeConfigFormValue(state, ["models", "providers", id]),
